...
M0<-glmer(FS~1+(1|Block),data=Saplings3,family="poisson")
M1<-glmer(FS~Year2*Collapse3+(1|Block),data=Saplings3,family="poisson")
M2<-glmer(FS~Year2+Collapse3+(1|Block),data=Saplings3,family="poisson")
M3<-glmer(FS~Collapse3+(1|Block),data=Saplings3,family="poisson")
M4<-glmer(FS~Year2+(1|Block),data=Saplings3,family="poisson")
...
new.data$FS[1]<-Coefs2[1,2]
new.data$UCI[1]<-new.data$FS[1]+(1.96*Coefs2[1,3])
new.data$LCI[1]<-new.data$FS[1]-(1.96*Coefs2[1,3])

for (j in 2:3){
  new.data$FS[j]<-Coefs2[1,2]+Coefs2[j,2]
  new.data$UCI[j]<-new.data$FS[j]+(1.96*Coefs2[j,3])
  new.data$LCI[j]<-new.data$FS[j]-(1.96*Coefs2[j,3])
}
new.data$FS[4]<-Coefs2[1,2]+Coefs2[2,2]+Coefs2[4,2]
new.data$UCI[4]<-new.data$FS[4]+(1.96*Coefs2[4,3])
new.data$LCI[4]<-new.data$FS[4]-(1.96*Coefs2[4,3])

#now plot these results
theme_set(theme_bw(base_size=12))
dodge <- position_dodge(width=0.9)
Sapling_plot<-ggplot(new.data,aes(x=Year2,y=(exp(FS)),ymin=(exp(LCI)),ymax=(exp(UCI)),fill=Collapse3))+
